...
const KarmaRankingsSidebar = ({ isOpen, onClose }) => {
  ...
  const loadMoreUsers = () => {
    ...
    setTimeout(() => {
      ...
      const nextBatch = allUsers.slice(currentCount, currentCount + USERS_PER_BATCH);
  ...
  const loadCompactRankings = async () => {
    ...
    try {
      ...
      if (cacheError) {
        console.warn('⚠️ Error cargando rankings cached, fallback a cálculo en tiempo real:', cacheError);
  ...
  const loadRealTimeRankings = async () => {
    try {
      ...
      try {
        ...
        if (rpcError) {
          console.warn('🔧 RPC function not available, using direct query (limited by RLS):', rpcError.code);
      ...
      if (votesError) console.warn('Error loading votes for sidebar:', votesError);
      ...
      if (commentsError) console.warn('Error loading comments for sidebar:', commentsError);

      // Obtener información de usuarios para los nombres (incluye votantes, escritores y comentaristas)
      let usersData = [];
      if ((stories && stories.length > 0) || (votes && votes.length > 0) || (comments && comments.length > 0)) {
  ...
  const calculateUserKarmaCompact = (stories, votes, comments, contests, users) => {
    const userKarma = {};
    ...
    comments.forEach(comment => {
      ...
      if (storyAuthorId && storyAuthorId !== commentAuthorId) {
        ...
        if (commentDate.getMonth() === currentMonth && commentDate.getFullYear() === currentYear) {
          userKarma[storyAuthorId].monthlyKarma += KARMA_POINTS.COMMENT_RECEIVED;
        }
      }
    });
        ...
        >
          {loading ? (
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-600 dark:text-dark-300">Calculando karma...</p>
            </div>
          ) : (
            <div className="p-4 space-y-4">
              ...
              <div className="text-center pb-2 border-b border-gray-200 dark:border-dark-600">
                <p className="text-sm text-gray-600 dark:text-dark-300">
                  ...
                  {displayedUsers.length < allUsers.length && ` de ${allUsers.length}`} escritores
                </p>
                  ...
                  <>
                    ...
                    {loadingMore && (
                      <div className="p-4 text-center">
                        ...
                        <p className="text-xs text-gray-500 dark:text-dark-400">Cargando más escritores...</p>
                      </div>
                    )}
                    ...
                    {!loadingMore && displayedUsers.length >= allUsers.length && allUsers.length > USERS_PER_BATCH && (
              ...
              <div className="bg-primary-50 dark:bg-primary-900/20 rounded-lg p-4 border border-primary-200 dark:border-primary-800">
                ...
                <div className="text-xs text-primary-800 dark:text-primary-300 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span>Publicar historia: <strong>+15</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <MessageCircle className="h-3 w-3" />
                    <span>Dar comentario: <strong>+2</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <MessageSquare className="h-3 w-3" />
                    <span>Recibir comentario: <strong>+3</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Heart className="h-3 w-3" />
                    <span>Recibir like: <strong>+2</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Vote className="h-3 w-3" />
                    <span>Votar: <strong>+1</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              {/* CTA motivacional */}
              <div className="bg-gradient-to-r from-primary-50 to-accent-50 dark:from-primary-900/20 dark:to-accent-900/20 rounded-lg p-4 text-center">
                <p className="text-sm text-gray-700 dark:text-dark-300 font-medium mb-3">
